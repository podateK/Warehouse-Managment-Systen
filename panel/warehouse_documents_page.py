...
class WarehouseDocumentsPage(QWidget):
    ...
    def generate_pdf_for_selected(self):
        ...
        for index in selected_rows:
            ...
            try:
                doc_id = int(doc_id_item.text())
                pdf_path = generate_pdf_from_database(self.db_manager, doc_id, output_dir="invoices")
                if pdf_path:
                    success_count += 1
                else:
                    error_count += 1
            except Exception as e:
                print(f"Błąd generowania PDF dla dokumentu: {e}")
                error_count += 1

        if success_count > 0:
            FloatingMessage.display(self, f"✅ Wygenerowano PDF: {success_count}", duration=3000)

            summary = f"Wygenerowano {success_count} PDF(ów)."
            if error_count > 0:
                summary += f"\nNie udało się wygenerować: {error_count}."
            summary += "\n\nFolder invoices zostanie otwarty."
            QMessageBox.information(self, "Sukces", summary)

            self.open_invoices_folder()
        else:
            QMessageBox.critical(self, "Błąd", "Nie udało się wygenerować żadnego PDF-a.")
